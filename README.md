...
[Features](#key-features) · [Architecture](#system-architecture) · [Quick Start](#quick-start) · [Evaluation](#evaluation-and-results) · [Documentation](#documentation)
...
The repository represents a complete local prototype with a Streamlit interface, two YOLO model weights, OCR integration, research artifacts, documentation, smoke tests, and GitHub Actions validation. It does **not** contain a REST API, database, Docker setup, authentication, cloud deployment, or production monitoring; those capabilities are therefore not claimed.
...
No verified public live deployment is currently available. Follow the [Quick start](#quick-start) instructions to run the application locally.
...
`app.py` is the verified Streamlit entry point. The repository can be configured for Streamlit Community Cloud without application secrets, but a successful public deployment has not been verified. PyTorch, EasyOCR model downloads, long-running video jobs, and large files may exceed hosted resource limits. Refer to [docs/deployment.md](docs/deployment.md) before deployment.
